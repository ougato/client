...
const ANDROID_SQLITE_DB_FILE_PATH: string = "org/cocos2dx/javascript/db/SQLiteDB";
    ...
    public async init(dbName: string, dbVersion: number = 1): Promise<boolean> {
        return new Promise((resolve: (value: boolean | PromiseLike<boolean>) => void, reject: (reason?: any) => void) => {
            ...
            if (cc.sys.os === cc.sys.OS_ANDROID) {
                isOK = jsb.reflection.callStaticMethod(ANDROID_SQLITE_DB_FILE_PATH, "init", "(Ljava/lang/String;ILjava/lang/String;)Z", dbName, dbVersion, JSON.stringify(DBConfig.Struct));
